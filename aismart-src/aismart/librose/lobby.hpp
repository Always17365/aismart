...
public:
	...
	{
	public:
		class tprogress_lock
		{
		public:
			tprogress_lock(thttp_sock& sock, gui2::tprogress_& progress)
				: sock_(sock)
			{
				VALIDATE(!sock_.progress_, null_str);
				sock_.progress_ = &progress;
			}
			~tprogress_lock()
			{
				sock_.progress_ = nullptr;
			}
		...
		bool network_connect_dialog(display& disp, bool quiet);
		bool network_receive_dialog(display& disp, int hidden_ms = 3);
		bool network_send_dialog(display& disp, const char* buf, int len, int hidden_ms = 3);
