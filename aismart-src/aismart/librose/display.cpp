...
display* display::create_dummy_display(CVideo& video)
{
	...
	return new display(null_str, NULL, video, &dummy_map, 0, ZOOM_72);
}
...
{
	return in_theme()? main_map_view_rect(): anim2::rt.rect;
}
...
void display::undraw_window_anim()
{
	if (area_anims_.empty() || in_theme()) {
		return;
	}
