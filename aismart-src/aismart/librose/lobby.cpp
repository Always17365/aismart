...
{
	posix_print("tsock::reset_connect()------, state_: %i\n", state_);
...
{
	posix_print("thttp_sock::reset_connect()------, socket_: %p\n", socket_.get());
...
{
	posix_print("tlobby::thttp_sock::mini_connected()------, state_: %i\n", state_);

	if (progress_) {
		progress_->set_percentage(gui2::tprogress_::finish_precentage);
	}
	response_size_ = 0;
	state_ = s_ready;
}
...
static std::string form_connect_to_title()
{
	...
	strstr << ht::generate_format(game_config::bbs_server.name, color_to_uint32(font::GOOD_COLOR));
...
bool tlobby::thttp_sock::network_connect_dialog(display& disp, bool quiet)
{
	VALIDATE(state_ == s_none || state_ == s_created, null_str);

	gui2::tprogress dlg(form_connect_to_title(), NULL, 4, true);
	tprogress_lock lock(*this, dlg);
	dlg.show(disp.video());
...
bool tlobby::thttp_sock::network_receive_dialog(display& disp, int hidden_ms)
{
	VALIDATE(state_ == s_ready, null_str);

	gui2::tprogress dlg("", NULL, hidden_ms, true);
	tprogress_lock lock(*this, dlg);
	dlg.show(disp.video());
...
bool tlobby::thttp_sock::network_send_dialog(display& disp, const char* buf, int len, int hidden_ms)
{
	VALIDATE(state_ == s_ready, null_str);
