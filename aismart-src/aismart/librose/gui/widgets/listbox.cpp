...
tlistbox::tlistbox(const std::vector<tlinked_group>& linked_groups)
	...
	, longpress_at_(twidget::npos)
	, longpress_ticks_(0)
	, last_longpress_at_(twidget::npos)
...
ttoggle_panel& tlistbox::insert_row(const std::map<std::string, std::string>& data, int at)
{
	if (gc_locked_at_ != twidget::npos) {
		...
		// since insert_row/erase_row should udpate gc_locked_at_ always, here set twidget::npos first.
...
void tlistbox::left_drag_grid_set_widget_visible(const std::map<std::string, tvisible>& visibles)
{
	...
	// when drag_at_ != twidget::npos, call twidget.set_visible direct.
	VALIDATE(drag_at_ == twidget::npos, null_str);

	twindow::tinvalidate_layout_blocker blocker(*get_window());
...
void tlistbox::erase_row(int at)
{
	if (gc_locked_at_ != twidget::npos) {
		...
		// 2. next erase_row, only one row will be erase. it should set gc_locked_at_ to twidget::npos.
		//    since erase_row should udpate gc_locked_at_ always, here set twidget::npos first.
		gc_locked_at_ = twidget::npos;
	}
...
void tlistbox::set_row_child_label(int at, const std::string& id, const std::string& label)
{
	...
	{
		// multiline label/button maybe trigger invalidate layout.
		twindow::tinvalidate_layout_blocker block(*this->get_window());
...
void tlistbox::mini_set_content_grid_origin(const tpoint& origin, const tpoint& content_origin)
{
	...
	VALIDATE(drag_at_ == twidget::npos && longpress_at_ == twidget::npos, "please call cancel_drag() before popup new window.");
}
...
{
	VALIDATE(longpress_at_ != twidget::npos, null_str);
...
{
	if (drag_judge_at_ == twidget::npos || longpress_at_ != twidget::npos) {
		return;
	}

	twindow* window = get_window();
	const twidget* current_mouse_focus = window->mouse_focus_widget();
...
{
	VALIDATE(drag_judge_at_ == twidget::npos && longpress_at_ >= 0 && longpress_at_ < list_grid_->children_vsize(), null_str);
...
void tlistbox::mini_mouse_down(const tpoint& first)
{
	...
	if (drag_at_ == twidget::npos && longpress_at_ == twidget::npos) {
		...
		if (use_drag_) {
			...
			longpress_timer_.reset(add_timer(100, boost::bind(&tlistbox::longpress_timer_handler, this)));
		}
		drag_judge_at_ = in_which_row(first.x, first.y);
		if (drag_judge_at_ == twidget::npos) {
			// content_grid maybe less than content, mouse maybe press at bonus part.
			return;
		}
		if (use_drag_ && did_can_drag_ && !did_can_drag_(*this, row_panel(drag_judge_at_))) {
			drag_judge_at_ = twidget::npos;
		}
...
{ 
	if (drag_judge_at_ == twidget::npos && drag_at_ == twidget::npos && longpress_at_ == twidget::npos) {
		return true;
	}
	if (drag_judge_at_ != twidget::npos) {
		// judging...
