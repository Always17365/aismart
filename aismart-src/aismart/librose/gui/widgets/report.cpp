...
void treport::erase_item(int at, bool auto_select)
{
...
void treport::validate_children_continuous() const
{
	...
	for (int at = 0; at < size; at ++) {
		...
		if (at < front_childs_) {
			VALIDATE(widget.at_ == twidget::npos, null_str);
...
void treport::set_item_visible(int at, bool visible, bool auto_select)
{
...
void treport::set_item_visible(const tcontrol& widget, bool visible, bool auto_select)
{
	gui2::tgrid::tchild* children = content_grid()->children();
	...
	for (int n = front_childs_; n < childs; n ++) {
		...
		set_item_visible(n - front_childs_, visible, auto_select);
		return;
	}
...
void treport::set_item_label(int at, const std::string& label)
{
	gui2::tgrid::tchild* children = content_grid()->children();
	const int childs = content_grid()->children_vsize() - back_childs_;
	VALIDATE(at >= 0 && at < childs, null_str);
...
void treport::set_item_label(tcontrol& widget, const std::string& label)
{
	if (widget.label() == label) {
		return;
	}
