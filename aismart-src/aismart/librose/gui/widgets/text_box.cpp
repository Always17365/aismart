...
void ttext_box::set_cipher(const bool cipher)
{
	...
	std::string before;
	if (selection_start_.x != twidget::npos) {
...
void ttext_box::goto_start_of_line(const bool select)
{
	...
	SDL_Rect offset = integrate_->holden_rect(0, selection_end_.x != twidget::npos? selection_end_.y: selection_start_.y);
	set_cursor(offset, select); 
}
...
void ttext_box::goto_end_of_line(const bool select)
{
	...
	SDL_Rect end = integrate_->editable_at(get_width(), selection_end_.x != twidget::npos? selection_end_.y: selection_start_.y);
	set_cursor(end, select);
}
...
{
	posix_print("ttext_box::clear_selection() is called\n");
...
void ttext_box::handle_key_left_arrow(SDL_Keymod modifier, bool& handled)
{
	...
	if (selection_end_.x != twidget::npos && !(modifier & KMOD_SHIFT)) {
		clear_selection();
	}
...
void ttext_box::handle_mouse_selection(const tpoint& coordinate, const bool down)
{
	...
	if (hold_rect_is_valid(offset) && (!down || selection_end_.x == twidget::npos)) {
...
void ttext_box::signal_handler_mouse_motion(const event::tevent event, bool& handled, const tpoint& coordinate, const tpoint& coordinate2)
{
	...
	if (selectioning_) {
		...
		if (!multi_line_ && get_integrate_size().x > tcontrol::get_text_maximum_width()) {
