...
void tdistributor::capture_mouse(twidget* widget)
{
	...
	const twidget* original_capture_mouse = mouse_focus_widget();
	if (!widget) {
...
/*
	posix_print("tdistributor::signal_handler_sdl_mouse_motion------(%i, %i), mouse_captured_[%s, mouse_focus_[%p(%s), parent[(%s)\n", 
...
void tdistributor::mouse_motion(twidget* mouse_over, const tpoint& coordinate, const tpoint& coordinate2)
{
	VALIDATE(mouse_over, null_str);
/*
	posix_print("tdistributor::mouse_motion, coordinate(%i, %i), mouse_captured_ = %s, mouse_over: %s\n", 
		...
		, const tpoint& coordinate)
{
	const int times = times_ ++;
/*
	posix_print("[#%i]signal_handler_sdl_button_up[#%i]------mouse_focus_: %p(%s), window_id(%s)\n",
		times, button_,
