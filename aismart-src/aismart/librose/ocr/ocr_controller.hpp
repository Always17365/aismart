...
{
public:
	ocr_controller(const config &app_cfg, CVideo& video, surface& surf, std::vector<std::unique_ptr<tocr_line> >& lines, const std::vector<std::string>& fields, const std::string& pb_path);
	~ocr_controller();
...
private:
	...
	void recognize();
	void did_recognize(gui2::tprogress_& progress);
