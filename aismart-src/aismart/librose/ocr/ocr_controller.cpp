...
ocr_controller::ocr_controller(const config& app_cfg, CVideo& video, surface& surf, std::vector<std::unique_ptr<tocr_line> >& lines, const std::vector<std::string>& fields, const std::string& pb_path)
	...
	, lines_(lines)
	, fields_(fields)
	, pb_path_(game_config::path + "/" + game_config::generate_app_dir(game_config::app) + "/tensorflow/" + pb_path)
	, dragging_field_(gui2::twidget::npos)
...
{
	VALIDATE(dragging_field_ != gui2::twidget::npos, null_str);
...
void ocr_controller::did_recognize(gui2::tprogress_& progress)
{
	...
	for (int at = 0; at < size; at ++) {
		...
		for (std::set<trect>::const_iterator it2 = line.chars.begin(); it2 != line.chars.end(); ++ it2, char_at ++) {
			progress.set_percentage(100 * recognized_chars / total_chars);

			cv::Rect clip_rect(it2->x, it2->y, it2->w, it2->h);
			cv::Mat tmp;
...
/*
			if (char_at == 6) {
				...
				save_surface_to_file(tmp2, game_config::preferences_dir + "/3.png");
			}
*/
			uint32_t used_ticks_one = 0;
			wchar_t wch = tensorflow2::inference_char(current_session_, pb_path_, tmp, &used_ticks_one);
	...
	progress.set_percentage(gui2::tprogress_::finish_precentage);
}
...
std::map<std::string, tocr_result> ocr(const config& app_cfg, display& disp, const surface& surf, const std::vector<std::string>& fields, const std::string& pb_path)
{
	...
	surface target;
	{
		...
		for (std::vector<std::unique_ptr<tocr_line> >::const_iterator it = lines.begin(); it != lines.end(); ++ it) {
			...
			std::set<trect> chars = line.chars;
			line.chars.clear();
			for (std::set<trect>::const_iterator it2 = chars.begin(); it2 != chars.end(); ++ it2) {
				trect rect = *it2;
				rect.x += offset.x;
				rect.y += offset.y;
	...
	ocr_controller controller(app_cfg, disp.video(), target, lines, fields, pb_path);
